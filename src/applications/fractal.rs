...
impl Application {
    pub fn new() -> Self {
        ...
        Application {
            ...
            input: Arc::new(Mutex::new(input::Handler::new(&js.window().unwrap()))),
    ...
    pub fn init(&mut self) {
        let mut closure = || -> Result<(), JsValue> {
            self.input.lock().unwrap().hook(false);
            ...
            w_canvas.style().set_property("width", "720");
            w_canvas.style().set_property("height", "720");
            ...
            w_shader.attach(shader::ShaderType::Fragment, include_str!("./shader/fractal-3d.frag"));
            w_shader.link();
            ...
            w_shader.uniform1i_name("tex_sampler", 0);
            ...
            w_context.tex_parameteri(WebGl2::TEXTURE_2D, WebGl2::TEXTURE_WRAP_T, WebGl2::CLAMP_TO_EDGE as i32);
            w_context.tex_parameteri(WebGl2::TEXTURE_2D, WebGl2::TEXTURE_MIN_FILTER, WebGl2::LINEAR as i32);
            w_shader.uniform1i_name("texture_sampler", 1);
                            ...
                            let temp_callback = Closure::wrap(Box::new(move |media_stream: JsValue| {
                                ...
                                match media_stream.clone().dyn_into::<web_sys::MediaStream>() {
                                    ...
                                    Err(error) => match error.as_string() {
                                        Some(error) => log_error!("Media stream not available. You need a secure connection. Error info:\n{}", error),
                                        None => log_error!("Media stream not available. You need a secure connection."),
                                    }
                                }
            ...
            let req_anim_frame = |callback: &Closure<dyn FnMut()>| {
            ...
            *main_loop.borrow_mut() = Some(Closure::wrap(Box::new(move || {
                ...
                w_context.clear_color(0.0, 1.0, 0.0, 1.0);
                ...
                xvel = 0.0;
                yvel = 0.0;
                w_shader.uniform2f_name("view_offset", xpos as f32, ypos as f32);
                w_shader.uniform2f_name("offset", xpos2 as f32, ypos2 as f32);
                w_shader.uniform2f_name("z_size", z_canvas.width() as f32, z_canvas.height() as f32);
                w_shader.uniform2f_name("w_size", w_canvas.width() as f32, w_canvas.height() as f32);
                w_shader.uniform2f_name("zoom", xzoom as f32, yzoom as f32);

                if lock.vkey_down("KeyO") {
                    fractal_count += 0.01 * delta_time;
                ...
                w_shader.uniform1f_name("time", (js.time_now() / 1000.0) as f32);
                //w_shader.uniform1i_name("max_iterations", max_iters);
                //w_shader.uniform2f_name("offset", xpos as f32, ypos as f32);
                //w_shader.uniform2f_name("center", xpos2 as f32, ypos2 as f32);
                //w_shader.uniform2f_name("zoom", xzoom as f32, yzoom as f32);
                //w_shader.uniform1f_name("aspect_ratio", 1.0); // TODO:
                //w_shader.uniform1f_name("escape_radius", escape_radius as f32);
                //w_shader.uniform1f_name("fractal_count", (fractal_count * 2.0) as f32);
                //w_shader.uniform1i_name("flags", 0x0 | 0x4);
                //w_shader.uniform1i_name("coloring", 0x1);
                ...
                //if self.custom_framerate.0 {
                //    
                //}
                // THIS IS TEMP!!!
                req_anim_frame(self_loop.borrow().as_ref().unwrap());
            }) as Box<dyn FnMut()>));
        ...
        match closure() {
            Ok(_) => (),
            Err(error) => log_error!("initialization error:\n{:?}", error),
        };
    }
    pub fn start() {
