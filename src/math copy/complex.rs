...
impl Complex {
    ...
    pub fn powi(self, exponent: i32) -> Self {
        // De Moivre's formula
        // arg(z^n) = n*arg(z)
        // z' = cos(theta) + sin(theta)i
        let theta = self.arg() * exponent as f64;
        let radius = (self.re * self.re + self.im * self.im).sqrt().powi(exponent);
        return Self::new(
            theta.sin() * radius,
            theta.cos() * radius,
        )
    ...
    pub fn pow(self, exponent: Self) -> Self {
        ...
        if exponent.is_real() {
            ...
            if exponent.re.fract() == 0.0 {
                return self.powi(exponent.re as i32);
            }
        }
...
impl Complex {
    pub fn to_str_phasor(&self) -> String {
        let radius = self.abs();
        let theta = self.arg().to_degrees();
        format!("{:.4} ∠ {:.4}°", radius, theta)
    }
    pub fn to_str_polar(&self) -> String {
        let radius = self.abs();
        let theta = self.arg().to_degrees();
        format!("{:.4} * cos({:.4}°) + sin({:.4}°)i", radius, theta, theta)
    }
    pub fn to_str_exponential(&self) -> String {
        let radius = self.abs();
        let theta = self.arg().to_degrees();
